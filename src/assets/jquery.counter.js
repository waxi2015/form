...
(function($) {
	...
	$.fn.extend({
		counter: function(options) {
			...
			if(this.length > 1){
				counterBindedObjects = {};
				this.each(function(){$(this).counter(options)});
				return this;
			}
			...
			// @bugfix - inline attributes
			if (element.attr(options.namespace + '-charlimit-max') !== undefined) {
				defaults.goal = element.attr(options.namespace + '-charlimit-max');
			}
			if (element.attr(options.namespace + '-charlimit-chars-left') !== undefined) {
				defaults.msg = element.attr(options.namespace + '-charlimit-chars-left');
			}
			...
			if (options.inlineAttributes !== false) {
				D(options.inlineAttributes);
				if (options.inlineAttributes.goal !== undefined) {
					options.goal = element.attr(options.inlineAttributes.goal);
				}
				defaults.inlineAttributes = false;
			}
			...
			var methods = {
				init: function($obj) {
					...
					// create unique id for multiple[] elements
					if (objID.substr(objID.length - 2) == '[]') {
						objID = objID.substr(0, objID.length - 2);
						if (counterBindedObjects.hasOwnProperty(objID)) {
							var counter = counterBindedObjects[objID];
							counter++;
							counterBindedObjects[objID] = counter;
							objID+= '-' + counter;
						} else {
							counterBindedObjects[objID] = 1;
							objID+= '-1';
						}
					}

					var counterID = objID + '_count';

					// Check if unlimited typing is enabled
					methods.isLimitless();

					// Remove element if it was initialized before
					if ($(element).parent().find('.' + options.className).length) {
						$(element).parent().find('.' + options.className).remove();
					}

					// Insert counter after or before text area/box
					$countObj = $("<span id=" + counterID + "/>");
					var counterDiv = $('<div/>').attr('id', objID + '_counter').append($countObj).attr('class', options.className).append(" " + methods.setMsg());
